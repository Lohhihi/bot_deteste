	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ala @${num.split('@')[0]}\nMais um corno *${mdata.subject}*\n\nEspero que não enche o saco`
				...
				teks = `Vaza @${num.split('@')[0]} Conta até dez`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD ANJO BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] ANJO? Este é um recurso especial para o Anjo❌',
					ownerB: '[❗] ANJO? Este é um recurso especial para o Anjo❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de ANJO. Registre-se para fazer amizade com o bot Anjo por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Causs|17\n\n──「 ANJO  BOT 」──`,
			...
			const ownerNumber = ["5514 996551527@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5514996551527@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5514996551527@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5514996551527@s.whatsapp.net","5563974008589@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5514996551527@s.whatsapp.net","5563974008589@s.whatsapp.net"] 
...
axios.get(`https://www.receitaws.com.br/v1/cnpj/${aris}`).then((res) => {
	conn.sendMessage(id, '[❗] ESPERE PORRA@ ESTOU BUSCANDO DADOS', MessageType.text)
...
axios.get(`http://geradorapp.com/api/v1/cpf/generate?token=${aris}`).then((res) => {
	conn.sendMessage(id, '[❗] ESPERE CARALHO ESTOU BUSCANDO DADOS', MessageType.text)
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('cara, nao fale essas coisas seu noiado, é errado, mas vc e admin n irei te banir')
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("cadebot")){
			client.updatePresence(from, Presence.composing)
			reply("olha eu aqui carai fi duma égua")
	}
	
		if (messagesC.includes("bot")){
			client.updatePresence(from, Presence.composing)
			reply("solta a voz sabiar")
	}
